#!/usr/bin/env bash
# Seed the registry with the test plugin (devsper-plugin-demo) so you can
# run: pip install --index-url http://localhost:3000/simple/ devsper-plugin-demo
#
# Requires: API running with storage (S3 or local) configured, and a built wheel.
# Usage:
#   ./registry/scripts/seed-dev.sh
#   ./registry/scripts/seed-dev.sh /path/to/devsper_plugin_demo-0.1.0-py3-none-any.whl
#
...
PKG_NAME="devsper-plugin-demo"
WHEEL="${1:-}"

if [ -z "$WHEEL" ]; then
  WHEEL="registry/test-plugin/dist/devsper_plugin_demo-0.1.0-py3-none-any.whl"
  if [ ! -f "$WHEEL" ]; then
...
curl -s -X POST "$BASE/api/v1/packages" \
  ...
  -H "Content-Type: application/json" \
  -d "{\"name\":\"$PKG_NAME\",\"display_name\":\"Devsper Plugin Demo\",\"description\":\"Demo plugin for registry testing\"}" || true
